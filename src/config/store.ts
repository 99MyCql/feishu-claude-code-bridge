...
export async function ensureSecretsGetterWrapper(): Promise<string> {
  ...
  const content = isWin
    ? // .cmd wrapper — Windows CreateProcess cannot execute a bare shell script.
      // @ prefix suppresses echo; %* forwards all arguments through to the
      // bridge subcommand so lark-cli's exec-provider audit sees a direct
      // invocation rather than a symlink/root-owned node binary.
      `@"${node}" "${bridgeEntry}" secrets get %*\r\n`
    : // POSIX shell wrapper — single-quote paths so the wrapper passes
      // lark-cli's AssertSecurePath audit (always user-owned, never a
      // symlink) even when node is a Homebrew/Volta symlink or root-owned.
      (() => {
